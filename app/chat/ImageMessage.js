    ...
    return (
      <View style={[isSelf ? styles.right : styles.left]}>
        <TouchableOpacity
          ...
          activeOpacity={1}
          disabled={isOpen}
          onPress={() => this.props.onMessagePress('image', parseInt(this.props.rowId), message.per.content.uri)}
          style={{backgroundColor: 'transparent', padding: 5, borderRadius: 5}}
          onLongPress={() => {
            this.props.onMessageLongPress(this[`item_${this.props.rowId}`], 'image', parseInt(this.props.rowId), message.per.content.uri)
          }} >
